...
/// Walk a Thrift struct's fields, dispatching on id; unmatched ids skip.
/// `$f` exposes `f.id`, `f.field_type`, `f.bool_val` inside each arm.
macro_rules! read_struct_fields {
    ($prot:ident, |$f:ident| { $($id:literal => $arm:expr),* $(,)? }) => {
...
fn read_row_group(
    ...
    // Inlined skips at ids 5/6/7 (file_offset, total_compressed_size, ordinal):
    // no in-tree consumer; bypass the generic recursive `skip_till_depth`.
    read_struct_fields!(prot, |f| {
...
fn read_column_meta_data(
    ...
    // Inlined skips at ids 1/2/3 (type_, encodings, path_in_schema): universally
    // present, no in-tree consumer; bypass the generic recursive skip.
    // Ids 8 (key_value_metadata), 13 (encoding_stats), 16 (size_statistics),
    // 17 (geospatial): fall through to the generic skip arm. Dedicated
    // skip_simple_struct helpers tried for 13/16/17, no measurable win.
    read_struct_fields!(prot, |f| {
        1 => prot.skip_vlq()?,
        2 => prot.skip_list_of_varint()?,
        3 => prot.skip_list_of_binary()?,
...
fn read_statistics(
    ...
    // Ids 1/2 (deprecated max/min from pre-2019 writers): skip without alloc.
    // Ids 5/6 (max_value/min_value): record (offset, len) into the shared
